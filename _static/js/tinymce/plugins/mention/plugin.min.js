  ...
  // <script>
    ...
    AutoComplete.prototype = {
        ...
        process: function (data) {
            ...
            if (result.length) {
                this.$dropdown.html(result.join(''));
                var popupWidth = this.$dropdown.width();
                var current_pos = parseInt(this.$dropdown.css('left').replace('px', ''));
                var availWidth = $(document).width();

                if((current_pos + popupWidth) > availWidth) {
                    current_pos = current_pos - ((current_pos + popupWidth) - availWidth) - 20;
                }

                this.$dropdown.css('left', current_pos);
                this.$dropdown.removeClass('creating_max_width');
                this.$dropdown.show();

            } else {
                this.renderInlineCreate(this.query);
                //this.$dropdown.hide();
            }
        },

        renderInlineCreate: function(val) {
            this.$dropdown.html('<li class="inlinenew" data-email="" data-type="" data-id="0" data-name="' + this.query + '"><span>' + this.options.strings.create + ': <strong>' + this.query + '</strong> ' + this.options.strings.as + ' <a href="javascript:;" data-trigger="custom_link" data-type="person"><span class="label label-success">' + this.options.strings.person + '</span></a> <a href="javascript:;" data-trigger="custom_link" data-type="company"><span class="label label-info">' + this.options.strings.company + '</span></a></span><li>');
        },
        ...
        autoCompleteClick: function (e) { // aqui
            var item = $(e.target).closest('li').data();
            var newitem = $(e.target).closest('a').data()
            if(newitem.trigger == "custom_link") {
        ...
        offset: function () {
            ...
            var availWidth = $(document).width();
            var popupWidth = $(this.editor.getContainer()).width()
            var default_left = rtePosition.left + contentAreaPosition.left + nodePosition.left;
